    ...
    pub fn to_primitive_int_lax(value: DinoRef, vtype: u16, base: Option<u32>, memory: &mut MemoryManager) -> Result<i64, RuntimeError> {
        match vtype {
            ...
            value_type::STRING => {
                let offset = value.decode_index();
                let bytes = memory.get_const_bytes(offset);
                parse_lax::<i64>(bytes, base).map_err(|e| RuntimeError::Typed(RuntimeErrorType::NumericParse(e)))
            }
            value_type::BOOL => Ok(
    ...
    pub fn to_primitive_float_lax(value: DinoRef, vtype: u16, memory: &mut MemoryManager) -> Result<f64, RuntimeError> {
        match vtype {
            ...
            value_type::BIGINT => {
                ...
                let bytes = memory.get_const_bytes(offset);
                let bigint = BigInt::from_slice(bytes);
                bigint.to_f64().map_err(|e| RuntimeError::Typed(RuntimeErrorType::NumericParse(e)))
            }
            value_type::STRING => {
                let offset = value.decode_index();
                let bytes = memory.get_const_bytes(offset);
                parse_lax::<f64>(bytes, None).map_err(|e| RuntimeError::Typed(RuntimeErrorType::NumericParse(e)))
            }
            value_type::BOOL => Ok(
